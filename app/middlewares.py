...
class ProtectionMiddleware(BaseMiddleware):
    PROTECTED_HANDLER = {"travel_callback", "add_travel_callback", "get_travel_callback", "get_all_travels_callback",
                         "get_distance_callback", "get_country_callback", "delete_travel_callback",
                         "set_nickname_callback", "delete_user_callback"}
    ...
    async def __call__(self,
        ...
        if handler_name in self.PROTECTED_HANDLER:
            if event.from_user:
                user = await storage.get_user(user_id=event.from_user.id)
                if not user or user.deleted_date:
                    await event.answer(
